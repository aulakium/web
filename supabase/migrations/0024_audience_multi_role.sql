-- ════════════════════════════════════════════════════════════════════════
-- Colequium — Audiencia: múltiples grados/salones + filtro por rol.
-- Al publicar se pueden elegir varios destinos (varias filas en `audiences`,
-- semántica OR) y además acotar a docentes / padres / ambos con
-- posts.audience_roles (NULL = todos los de la audiencia).
-- ════════════════════════════════════════════════════════════════════════
alter table public.posts
  add column if not exists audience_roles text[];   -- null = sin filtro de rol
...
returns table(
  ...
  comments_enabled boolean, cover_url text
