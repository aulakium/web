-- ════════════════════════════════════════════════════════════════════════
-- Colequium — feed() para el filtro global "ver por hijo":
--   • Devuelve group_id (salón al que apunta el aviso, o null = toda la escuela).
--   • Acepta p_group: si viene, muestra SOLO los avisos de ese salón + los de
--     toda la escuela (sin salón). Null = sin filtro (todo).
-- ════════════════════════════════════════════════════════════════════════
...
create function public.feed(
  p_limit integer default 20, p_offset integer default 0, p_group uuid default null
)
returns table(
  ...
  event_location text, event_at timestamptz, task_action text, task_due timestamptz,
  task_done boolean, reads integer, my_rsvp text, rsvp_yes integer, group_id uuid
)
language sql stable security definer set search_path = public as $$
  ...
  visible as (
    ...
    where p.community_id = (select community_id from me)
      ...
      -- Filtro global por hijo: el salón elegido, o avisos de toda la escuela.
      and (
        p_group is null
